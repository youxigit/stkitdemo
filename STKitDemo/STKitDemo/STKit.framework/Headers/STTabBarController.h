...
@property(nonatomic,readonly) STTabBar *tabBar; // Provided for -[UIActionSheet showFromTabBar:]. Attempting to modify the contents of the tab bar directly will throw an exception.
...
@property(nonatomic, strong) STTabBarItem * customTabBarItem; // Automatically created lazily with the view controller's title if it's not set explicitly.

@property(nonatomic, readonly, strong) STTabBarController * customTabBarController; // If the view controller has a tab bar controller as its ancestor, return it. Returns nil otherwise.
