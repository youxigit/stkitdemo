...
- (id) initWithStyle:(UITableViewStyle)tableViewStyle {
    ...
    if (self) {
        ...
        STDTableViewCellItem * item00 = [[STDTableViewCellItem alloc] initWithTitle:@"导航适配" target:self action:@selector(testNavigationActionFired)];
        STDTableViewSectionItem * section0 = [[STDTableViewSectionItem alloc] initWithSectionTitle:@"包含对导航的一些适配，主要针对于一些NavigationBarHidden时，push或者pop的情况" items:@[item00]];
        [dataSource addObject:section0];
        
        STDTableViewCellItem * item10 = [[STDTableViewCellItem alloc] initWithTitle:@"查看大图" target:self action:@selector(imageDownloadActionFired)];
        ...
        STDTableViewSectionItem * section3 = [[STDTableViewSectionItem alloc] initWithSectionTitle:@"应用内的提醒, 适合即时通讯类的收到消息时提示" items:@[item30]];
