...
//    self.coverImageView.alpha = 0.9;
    ...
    UIBarButtonItem * resetItem = [[UIBarButtonItem alloc] initWithTitle:@"重置" style:UIBarButtonItemStylePlain target:self action:@selector(resetActionFired:)];
    ...
    UIBarButtonItem * finishItem = [[UIBarButtonItem alloc] initWithTitle:@"完成" style:UIBarButtonItemStylePlain target:self action:@selector(finishActionFired:)];
...
- (void) finishActionFired:(id) sender {
    ...
    CGContextRef context = CGBitmapContextCreate(0, blurImage.size.width, blurImage.size.height, 8, blurImage.size.width * 4, colorSpace, (CGBitmapInfo)kCGImageAlphaPremultipliedLast);
