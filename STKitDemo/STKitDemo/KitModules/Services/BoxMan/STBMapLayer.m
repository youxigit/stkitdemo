...
@interface STBMapLayer ()
/// 当前关数
@property (nonatomic, strong) STBLevel * level;
/// 地图中的数据
...
/// 搬运工，保证地图中只有一个
@property (nonatomic, strong) STBBoxMan * boxMan;
/// 所有的箱子
 ...
 * @return 返回两点的方向
 */
- (STBDirection) directionFromPoint:(CGPoint) start toPoint:(CGPoint) finish;
 ...
 */
- (SMoveType) canMoveWithDirection:(STBDirection) direction;
 ...
 */
- (void) pushManWithDirection:(STBDirection) direction indexPath:(SIndexPath) indexPath;
 ...
 * @param indexPath 当前位置在数组中的第几行第几列
 */
- (void) pushBoxWithDirection:(STBDirection) direction indexPath:(SIndexPath) indexPath;
 ...
 */
- (void) loadMapWithLevel:(STBLevel *) level {
    self.level = level;
    [self reloadMap];
}
...
- (void) loadMap {
    ...
    self.position = CGPointMake(width/2, 5);
        ...
        [rows enumerateObjectsUsingBlock:^(id obj2, NSUInteger idx2, BOOL *stop2) {
            ...
            if (type == SMapElementRedHouse) {
               ...
               // 绿色草垛
                ...
                // 搬运工 只有一个
                if (!self.boxMan) {
                    self.boxMan = [[[STBBoxMan alloc] initWithPosition:position] autorelease];
 ...
 */
- (STBDirection) directionFromPoint:(CGPoint) start toPoint:(CGPoint) finish {
    STBDirection direction = STBDirectionUnknown;
    if (!CGPointEqualToPoint(start, finish)) {
        ...
        if (fabsf(offsetX) >= fabsf(offsetY)) {
            ...
            if (offsetX > kBoxManLength) {
                direction =  STBDirectionRight;
            } else if(offsetX < -kBoxManLength){
                direction = STBDirectionLeft;
            }
        } else {
            if (offsetY > kBoxManLength) {
                direction =  STBDirectionDown;
            } else if (offsetY < -kBoxManLength){
                direction = STBDirectionUp;
            }
        }
    }
    return direction;
...
- (void) ccTouchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    STBDirection direction = [self directionFromPoint:self.start toPoint:self.finish];
    if (direction != STBDirectionUnknown) {
        self.isTouchEnabled = NO;
