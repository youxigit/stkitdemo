...
- (instancetype) initWithStyle:(UITableViewStyle)tableViewStyle {
    ...
    if (self) {
        ...
        STDTableViewSectionItem * section1 = [[STDTableViewSectionItem alloc] initWithSectionTitle:@"使用AudioQueue封装了音频的录音，播放等功能，可以用于实时通话中。并且实现了音频的可视化分析，使用OpenGLES对可视化的效果进行展示" items:@[item10]];
        ...
        STDTableViewSectionItem * section2 = [[STDTableViewSectionItem alloc] initWithSectionTitle:@"由于本人算法实在太差，所以在学习算法的过程中，用了一种可视化，可理解的方式进行了学习。当然重点还是iOS方面动画序列的处理" items:@[item20]];
