...
#import "STWaterStreamView.h"

#define kDefaultWaterCount 3
#define kMaxWaterCount 5

@interface STWaterStreamView () <UIGestureRecognizerDelegate>
...
//// 可见区域的index区间
@property (nonatomic, assign) STRange visibleRange;
...
@implementation STWaterStreamView
...
- (void) setDataSource:(id<STWaterStreamViewDataSource>)dataSource {
...
- (void) reloadData {
    ...
    [self.visibleViewDictionary enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
        UITableViewCell * streamCell = (UITableViewCell *)obj;
    ...
    if (self.streamHeaderView) {
        self.streamHeaderView.frame = CGRectMake(0, top, CGRectGetWidth(self.scrollView.bounds), self.streamHeaderView.height);
    ...
    if (self.streamFooterView) {
        self.streamFooterView.frame = CGRectMake(0, top, self.scrollView.width, self.streamFooterView.height);
...
- (void) relayoutSubViews {
    ...
    if (self.visibleViewDictionary.count == 0) {
        self.visibleRange = STRangeMake(0, self.elementCount);
    } else {
        ...
        self.visibleRange = STRangeMake(start, end);
    }
    ...
    for (int index = self.visibleRange.start; index < self.visibleRange.end; index ++) {
        ...
        UITableViewCell * streamCell = [self.visibleViewDictionary objectForKey:indexKey];
        if (!streamCell && CGRectIntersectsRect(visibleRect, rect)) {
            ...
            [self.scrollView sendSubviewToBack:streamCell];
...
- (void) inqueueReusableWithView:(UITableViewCell *) reuseView {
...
- (void) didSelectedStreamCell:(UITableViewCell *) streamCell index:(NSInteger) index {
    ...
    if (InRange(_visibleRange, _selectedIndex)) {
        NSInteger i = _selectedIndex - _visibleRange.start;
        UITableViewCell * cell1 = [self.visibleViewDictionary objectForKey:[NSString stringWithFormat:@"%d",i]];
        cell1.selected = NO;
    }
