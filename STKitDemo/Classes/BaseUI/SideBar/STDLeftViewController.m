...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil {
    ...
    if (self) {
        ...
        self.dataSource = @[@{
                                @"title":@"地图", @"image":@"SideBarNearby"
                                }, @{
                                @"title":@"组件", @"image":@"tab_receipt"
                                }, @{
                                @"title":@"服务", @"image":@"tab_service"
                                }, @{
                                @"title":@"我的", @"image":@"tab_profile"
                                }];
    }
    return self;
}
...
- (void) voiceDidRecognizerText:(NSNotification *) notification {
    ...
    if (![text contains:@"排"] || [text contains:@"行"]) {
        return;
    }
    if (self.sideBarController.sideAppeared) {
        ...
        if ([text contains:@"第一"]) {
            row = 0;
        } else if ([text contains:@"第二"]) {
            row = 1;
        }  else if ([text contains:@"第三"]) {
            row = 2;
        } else if ([text contains:@"第四"]) {
            row = 3;
        }
        if (row >= 0 && row < [self.tableView numberOfRowsInSection:0]) {
